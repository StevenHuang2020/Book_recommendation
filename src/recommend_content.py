...
#License: MIT License
"""
Description: Books recommendation, content basesd, similarity matrix.
...
def load_data(file):
    df = pd.read_csv(file, ) #encoding='unicode_escape', error_bad_lines=False
    #print(df.head())
    print(df.columns)
    print('df=\n', df)
    return df
...
def combine_features(data, columns): #columns=['title', 'authors']
    ...
    for i in range(data.shape[0]):
        ...
        features.append(str_tmp) #data['title'][i] + ' ' + data['authors'][i]
    return features
...
def do_dataset1():
    ...
    print('book simliarity scores:\n', scores[:100]) #show only the first 100 values
...
def do_dataset2():
    file = r'./db/books_new.csv' # books_only
    df = load_data(file)
