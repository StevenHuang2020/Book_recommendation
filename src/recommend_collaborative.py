...
Recommend according to the favorite of different users. There needs
a user-product rating table. When we need to recommend to users
other products that they may like, first get the favorite-similar users
by their historical product rating information. Then recommend the
products to users that the favorite-similar user browsed or rated.
This is just a typical collaborative filtering rule, and you can explore
or define more rules for your recommender system.
"""
...
def cosin_similarity(df, user_id_for):
    ...
    # remove the first score that is the user self
...
def compare_similar_users(df, sorted_scores, top_user=5):
    """get top similarities users"""
    # print('df, df.index=', df, df.index)
    list_users = []
    for item in sorted_scores[:top_user]:
...
def recommend_for_user(df, user_id_for, users, books, top=10):
    """
    recommend for user

    Args:
        df: data
        user_id_for (int): user id, must belongto the index of df
        users: user db
        books: books db
    ...
    df_users = compare_similar_users(df, cs, top_user=1)
    ...
    for i in range(df_users.shape[0]):
        ...
        print(i + 1, 'similar-favorite user, userId:', k.name, user['location'].values[0],
              user['age'].values[0].astype(int))
...
def rec_collaborative():
    ...
    recommend_for_user(df, user_id_for=3757, users=users, books=books, top=10)
